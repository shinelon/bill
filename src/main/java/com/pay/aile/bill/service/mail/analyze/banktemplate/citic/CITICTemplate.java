...
public class CITICTemplate extends AbstractCITICTemplate {
	...
	public void initRules() {
		...
		if (rules == null) {
			...
			rules.setDetails("\\d{8} \\d{8} \\d{0,4} \\d*\\D* [A-Za-z]+ -?\\d+.?\\d+ RMB -?\\d+.?\\d+");
		}
	}
	...
	protected void analyzeInternal(AnalyzeParamsModel apm) {
		...
		if (StringUtils.hasText(rules.getDueDate())) {
			...
			result = list.get(0);
			String date = result.replaceAll("年", "-").replaceAll("月", "-").replaceAll("日", "");
			sa = date.split("：");
			date = sa[sa.length - 1];
		...
		if (StringUtils.hasText(rules.getCurrentAmount())) {
			// 应还款额
			list = PatternMatcherUtil.getMatcher(rules.getCurrentAmount(), content);
			if (list.isEmpty()) {
